...
#
def to_adjacency_matrix(net, edgelist=False, shape=None):
    """Convert to the adjacency matrix in form of sparse.csr_matrix.

    :param net: adjacency matrix, or edge list of shape (N, 2) or (N, 3) where
                the optional third column contains edge weights.
    :type net: np.ndarray, csr_matrix, or list-like
    :param edgelist: if True, treat net as an edge list and convert accordingly.
    :type edgelist: bool
    :param shape: shape of the output matrix (rows, cols). Only used when
                  edgelist=True. If None, inferred from the max node index.
    ...
    if edgelist:
        net = np.asarray(net)
        logging.info("Converting edge list (%d edges) to csr_matrix.", len(net))
        return _edgelist_to_csr(net, shape)
    if sparse.issparse(net):
        if isinstance(net, sparse.csr_matrix):
            logging.info("Input is already csr_matrix, no conversion needed.")
            return net
        logging.info("Converting %s to csr_matrix.", type(net).__name__)
        return sparse.csr_matrix(net)
    elif isinstance(net, np.ndarray):
        logging.info("Converting dense numpy array %s to csr_matrix.", net.shape)
        return sparse.csr_matrix(net)
    else:
        raise ValueError("Unexpected data type {} for the adjacency matrix".format(type(net)))


def _edgelist_to_csr(edgelist, shape=None):
    """Convert an edge list array to a csr_matrix.

    :param edgelist: (N, 2) or (N, 3) array. Columns: [src, dst] or [src, dst, weight].
    :type edgelist: np.ndarray
    :param shape: shape of the output matrix. If None, inferred from max index.
    :type shape: tuple or None
    :return: adjacency matrix
    :rtype: sparse.csr_matrix
    """
    src = edgelist[:, 0].astype(np.int64)
    dst = edgelist[:, 1].astype(np.int64)
    ...
    if shape is None:
        ...
        shape = (n, n)
    return sparse.csr_matrix((weights, (src, dst)), shape=shape)
