...
class Node2VecTripletLoss(nn.Module):
    ...
    def forward(self, model, iwords, owords, nwords):
        ivectors = model.forward(iwords.to(model.device_in)).unsqueeze(2).to(model.device_out)
        ovectors = model.forward(owords.to(model.device_out), forward_in=False)
        nvectors = model.forward(nwords.to(model.device_out), forward_in=False).neg()
